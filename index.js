...
r.innerHTML='<select class="form-hb rule-field-sm action-type" onchange="toggleActionType(this)"><option value="tuya">Device</option><option value="notify">Notify</option></select>'
+'<span class="action-tuya-fields"><select class="form-hb rule-field action-device"><option value="">\\u2014 Device \\u2014</option>'+opts+'</select><select class="form-hb rule-field-sm action-value"><option value="true">ON</option><option value="false">OFF</option></select><input type="number" class="form-hb rule-field-sm action-duration" placeholder="Dur. min" min="0" /><input type="number" class="form-hb rule-field-sm action-interval" placeholder="Int. min" min="0" /></span>'
+'<span class="action-notify-fields" style="display:none"><input type="text" class="form-hb rule-field action-title" placeholder="Title" /><input type="text" class="form-hb rule-field action-message" placeholder="Message" /></span>'
+'<div class="rule-remove"><button class="btn-hb btn-hb-outline btn-hb-sm btn-hb-icon" onclick="this.closest(\\'.rule-row\\').remove()"><i class="bi bi-x"></i></button></div>';
c.appendChild(r);
}
function toggleActionType(sel){
const row=sel.closest('.rule-row');
const isNotify=sel.value==='notify';
const tf=row.querySelector('.action-tuya-fields');
const nf=row.querySelector('.action-notify-fields');
if(tf)tf.style.display=isNotify?'none':'';
if(nf)nf.style.display=isNotify?'':'none';
}
...
const typeSel=r.querySelector('.action-type');
const atype=typeSel?typeSel.value:'tuya';
if(atype==='notify'){
