...
import { parseBody, sendJson, sendText, setCookie, clearCookie, route, matchRoute } from './lib/router.js';
...
const { createRequestHandler, ensureCertificates } = serverState;
...
async function main() {
  ...
  let httpServer;
  if (tls) {
    httpServer = https.createServer({ cert: tls.cert, key: tls.key }, requestHandler);
    ...
    httpServer.listen(port, '0.0.0.0', () => {
      log.info('HTTP server listening on port ' + port + ' (not secure)');
    });
  }
