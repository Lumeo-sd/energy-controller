...
export function createAppState(DATA_DIR, loadConfig, saveConfig, decryptSecret, pushNotification) {
  ...
  function saveDevices() {
    try {
      const clean = tuyaDevices.map(d => ({ id: d.id, name: d.name, ip: d.ip || '', online: d.online || false, switch: d.switch, power: d.power || 0, voltage: d.voltage || 0, current: d.current || 0, group: d.group || '' }));
