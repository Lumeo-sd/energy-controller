...
export function createAppState(DATA_DIR, loadConfig, saveConfig, decryptSecret, pushNotification) {
  ...
  function markCloudQuotaExceeded(result) {
    ...
    const msg = 'Tuya cloud quota exceeded (code: ' + (result ? result.code : '?') + '), cloud fallback disabled until ' + retryAt;
    log.warn(msg);
    if (!_cloudQuotaNotified && pushNotification) {
      _cloudQuotaNotified = true;
      pushNotification('Tuya Quota Exceeded', msg).catch(() => {});
    }
  }
  ...
  async function syncDeviceNamesFromCloud() {
    if (!isCloudAvailable()) {
      log.info('Cloud unavailable (quota), skipping device sync');
      return;
    }
    const cfg = await loadConfig();
    const tc = cfg.tuya || {};
    try {
      ...
      if (result.quotaExceeded) { markCloudQuotaExceeded(result); return; }
        ...
        for (const cloudDev of cloudDevices) {
          if (!tuyaDevices.some(d => d.id === cloudDev.id)) {
            ...
            try {
              ...
              if (keyResult.quotaExceeded) { markCloudQuotaExceeded(keyResult); break; }
  ...
  let _cloudFallbackCount = 0;
  let _cloudFallbackWindowStart = 0;

  function applyCloudStatus(dev, result) {
  ...
  async function fetchDeviceStatuses() {
    const cfg = await loadConfig();
    const tc = cfg.tuya || {};
    const mode = (tc.controlMode) || 'auto';
    const cloudNeeded = mode !== 'local';
    let cloudFallbacks = 0;

    for (const dev of tuyaDevices) {
      let gotLocal = false;

      if ((mode === 'local' || mode === 'auto') && dev.ip && dev.localKey) {
        try {
          const local = getLocalDevice(dev);
          const data = await local.withLock(async () => await local.queryAll());
          if (data && data.dps) {
            ...
            if (data.dps['21'] !== undefined) dev.current = data.dps['21'];
          ...
          if (mode === 'local') {
            log.warn(dev.name + ': local-only mode, skipping cloud fallback');
            continue;
          }
        }
      } else if (mode === 'local') {
        log.warn(dev.name + ': no local connection (local-only mode)');
        continue;
      }

      if (!gotLocal && cloudNeeded && isCloudAvailable()) {
        try {
          if (!tuyaToken || !tuyaUid) await getTuyaTokenFunc();
          if (!tuyaUid) continue;
          const result = await tuyaRequest('GET', '/v1.0/devices/' + dev.id + '/status', null, tuyaToken, tc);
          if (result.quotaExceeded) { markCloudQuotaExceeded(result); continue; }
          if (applyCloudStatus(dev, result)) {
            cloudFallbacks++;
            log.info(dev.name + ': status fetched (cloud fallback, local was: ' + (dev.online ? 'unavailable' : 'no data') + ')');
          }
        } catch (err) {
          log.error('Cloud fallback failed for ' + dev.name + ': ' + err.message);
        }
      }
    }
    ...
    if (cloudFallbacks > 0) {
      const now = Date.now();
      if (now - _cloudFallbackWindowStart > 300000) {
        _cloudFallbackCount = 0;
        _cloudFallbackWindowStart = now;
      }
      _cloudFallbackCount += cloudFallbacks;
      log.info('Device statuses fetched (' + cloudFallbacks + '/' + tuyaDevices.length + ' via cloud fallback, ' + _cloudFallbackCount + ' in last 5min)');
      if (_cloudFallbackCount > 12) {
        log.warn('Frequent cloud fallback (' + _cloudFallbackCount + ' calls in 5min) — local connectivity may be degraded');
      }
    } else {
      log.info('Device statuses fetched (local)');
    }
  ...
  async function controlDevice(deviceId, value) {
    ...
    if (mode === 'local' || mode === 'auto') {
      if (device.ip && device.localKey) {
        try {
          ...
          device.switch = value;
          saveDevices();
          log.info(device.name + ' set to ' + (value ? 'ON' : 'OFF') + ' (local)');
          return;
        } catch (err) {
          if (mode === 'local') {
            throw new Error('Local control failed for ' + device.name + ': ' + err.message);
          }
          log.warn('Local control failed for ' + device.name + ': ' + err.message + ' — falling back to cloud');
        }
      } else if (mode === 'local') {
        throw new Error('No local key for ' + device.name + ' (local-only mode)');
      }
    }

    if (mode === 'local') {
      throw new Error('No local connection for ' + device.name + ' (local-only mode)');
    }

    if (!isCloudAvailable()) {
      throw new Error('Cloud unavailable (quota) for ' + device.name + ', local was unreachable');
    }

    const token = await getTuyaTokenForControl();
    if (!token) throw new Error('No local key and failed to get Tuya token');
    const tc = cfg.tuya || {};
    ...
    if (result.quotaExceeded) { markCloudQuotaExceeded(result); throw new Error('Cloud quota exceeded for ' + device.name); }
    if (result.success) {
      device.switch = value;
      saveDevices();
      log.info(device.name + ' set to ' + (value ? 'ON' : 'OFF') + ' (cloud)');
    } else {
  ...
  async function initTuya() {
    ...
    if (!discoveryRef) {
      discoveryRef = startDiscovery(tuyaDevices, () => { saveDevices(); });
    }
  }
