...
function markCloudQuotaExceeded(result) {
  ...
  async function fetchDeviceStatuses() {
      ...
      if (!gotLocal && cloudNeeded && isCloudAvailable()) {
        try {
          ...
          if (applyCloudStatus(dev, result)) {
            cloudFallbacks++;
            log.info(dev.name + ': status fetched (cloud fallback)');
          }
        } catch (err) {
    ...
    if (cloudFallbacks > 0) {
      ...
      log.info('Device statuses fetched (' + cloudFallbacks + '/' + tuyaDevices.length + ' via cloud fallback, ' + _cloudFallbackCount + ' in 5min)');
      if (_cloudFallbackCount > 12) {
        log.warn('Frequent cloud fallback (' + _cloudFallbackCount + ' in 5min) - local connectivity may be degraded');
      }
    } else {
