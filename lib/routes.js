export function registerRoutes(ctx) {
  ...
  route('POST', '/api/change-username', async (req, res) => {
    try {
      const { currentPassword, newUsername } = req.body || {};
      if (!currentPassword || !newUsername) return sendJson(res, 400, { success: false, message: 'Current password and new username required' });
      if (newUsername.length < 2) return sendJson(res, 400, { success: false, message: 'Username must be at least 2 characters' });
      const auth0 = await loadAuthFile();
      const curOk = verifyPassword(currentPassword || '', auth0.salt, auth0.hash);
      if (!curOk) return sendJson(res, 401, { success: false, message: 'Current password is incorrect' });
      const users = await loadUsers();
      if (newUsername === auth0.username) return sendJson(res, 400, { success: false, message: 'New username is the same as current' });
      if (users[newUsername]) return sendJson(res, 409, { success: false, message: 'Username already exists' });
      const oldUsername = auth0.username || 'admin';
      const oldDir = getUserPrefsDir(oldUsername);
      const newDir = getUserPrefsDir(newUsername);
      if (fs.existsSync(oldDir) && oldDir !== newDir) {
        await fs.promises.rename(oldDir, newDir);
      }
      auth0.username = newUsername;
      await fs.promises.writeFile(AUTH_FILE, JSON.stringify(auth0, null, 2), { mode: 0o600 });
      if (users[oldUsername]) {
        delete users[oldUsername];
        users[newUsername] = { role: 'admin', createdAt: Date.now() };
        await saveUsers(users);
      }
      clearSessions();
      log.info('Username changed from ' + oldUsername + ' to ' + newUsername + ', all sessions invalidated');
      sendJson(res, 200, { success: true });
    } catch (err) {
      sendJson(res, 500, { success: false, message: err.message });
    }
  ...
  route('POST', '/api/notifications/dismiss', async (req, res) => {
    try {
      ...
      if (b.title !== undefined) {
        _notifHistory.forEach(n => { if (n.title === b.title && n.type === b.type) n.dismissed = true; });
  ...
  route('POST', '/api/notifications/dismiss-all', async (req, res) => {
    ...
    if (b.id !== undefined) {
      ...
      _notifHistory.forEach(n => { if (n.title === b.title && n.type === b.type) n.read = true; });
    } else {
  ...
  async function saveUserPrefs(username, prefs) {
    ...
    await fs.promises.writeFile(getUserPrefsPath(username), JSON.stringify(prefs, null, 2), { mode: 0o600 });
  }
  ...
  route('POST', '/api/users', async (req, res) => {
    ...
    try {
      ...
      if (!fs.existsSync(getUserPrefsPath(username))) {
        await fs.promises.writeFile(getUserPrefsPath(username), JSON.stringify({ tileVis: {}, tileOrder: [], accent: 'purple', notifGroup: true }, null, 2), { mode: 0o600 });
      }
      sendJson(res, 200, { success: true });
    } catch (err) {
      sendJson(res, 500, { success: false, message: err.message });
  ...
  route('POST', '/api/system-update', (req, res) => {
    const action = req.body && req.body.action;
    if (action !== 'check' && action !== 'upgrade') {
      return sendJson(res, 400, { success: false, message: 'Invalid action' });
    }
    const cmd = action === 'check' ? 'sudo apt update' : 'sudo apt upgrade -y';
    exec(cmd, { timeout: 180000, maxBuffer: 1024 * 1024 }, (err, stdout, stderr) => {
      const output = (stdout || '') + (stderr || '');
      if (err) {
        return sendJson(res, 200, { success: true, action, output, hasErrors: true, message: err.message });
      }
      const upgradable = action === 'check' ? (output.match(/^(\d+) (packages can be upgraded|can be upgraded)/m) || [])[1] || '0' : null;
      sendJson(res, 200, { success: true, action, output, upgradable: parseInt(upgradable) || 0 });
    });
  });

  route('POST', '/api/system-reboot', (req, res) => {
    sendJson(res, 200, { success: true, message: 'Rebooting...' });
