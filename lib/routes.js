export function registerRoutes(ctx) {
  ...
  route('GET', '/api/status', async (req, res) => {
    ...
    const availWh = (soc / 100) * capacityWh;
    const runtimeMin = availWh / ((load / 0.965) + 45) * 60;
    const batteryEndsAt = inverterData.lastUpdate ? Date.now() + runtimeMin * 60000 : null;
  ...
  route('POST', '/api/plugin-config', async (req, res) => {
    try {
      ...
      if (newCfg.batteryCapacityWh !== undefined) merged.batteryCapacityWh = parseInt(newCfg.batteryCapacityWh) || 5120;
  ...
  route('POST', '/api/scenes', async (req, res) => {
    ...
    if (!scene.if || !Array.isArray(scene.if.conditions) || scene.if.conditions.length === 0) {
    ...
    for (const action of (scene.then && scene.then.actions) || []) {
      try {
        if (action.type === 'tuya') {
          ...
          results.push({ ok: true, action: 'tuya:' + action.device });
        } else if (action.type === 'notify') {
          pushNotification('Automation "' + scene.name + '" (manual)', action.message || 'Rule triggered', 'info');
          pushSceneTrace(scene.name, 'notify (manual)', action.message || '');
          results.push({ ok: true, action: 'notify' });
        } else if (action.type === 'webhook') {
          const url = action.url;
          if (url) {
            fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ scene: scene.name, triggered: true, ts: Date.now() }) }).catch(()=>{});
            pushSceneTrace(scene.name, 'webhook (manual)', url);
            results.push({ ok: true, action: 'webhook' });
          }
        } else if (action.type === 'priority') {
          pushNotification('Automation "' + scene.name + '" (manual)', 'Set priority: ' + action.value, 'info');
          pushSceneTrace(scene.name, 'priority (manual)', action.value);
          results.push({ ok: true, action: 'priority:' + action.value });
        } else if (action.type === 'delay') {
          pushSceneTrace(scene.name, 'delay (manual)', action.duration + 'min');
          results.push({ ok: true, action: 'delay:' + action.duration + 'min' });
        } else {
          await controlDevice(action.device, action.value);
          pushNotification('Automation "' + scene.name + '" (manual)', deviceName(action.device) + ' = ' + (action.value ? 'ON' : 'OFF'), 'info');
          pushSceneTrace(scene.name, 'apply (manual)', action.device + '=' + (action.value ? 'ON' : 'OFF'));
