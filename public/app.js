...
async function loadUserPrefsFromServer(){try{const r=await apiGet('/api/user-prefs');if(!r.success||!r.prefs)return;_currentUser=r.username||null;_currentRole=r.role||null;var pu=document.getElementById('profile-username');if(pu)pu.textContent=_currentUser||'admin';var pu2=document.getElementById('profile-username-page');if(pu2)pu2.textContent=_currentUser||'admin';var prl=document.getElementById('profile-role-label');if(prl)prl.textContent=_currentRole==='admin'?'Administrator':'Viewer';var prp=document.getElementById('profile-role-page');if(prp)prp.textContent=_currentRole==='admin'?'Administrator':'Viewer';if(_currentRole==='admin'){var pr=document.getElementById('profile-admin-row');if(pr)pr.style.display='';var pd=document.getElementById('profile-admin-divider');if(pd)pd.style.display='';loadUsersList();}else{var pr2=document.getElementById('profile-admin-row');if(pr2)pr2.style.display='none';var pd2=document.getElementById('profile-admin-divider');if(pd2)pd2.style.display='none';}const p=r.prefs;if(p.accent){localStorage.setItem('ecmAccent',p.accent);setAccent(p.accent,false);}if(p.notifGroup!==undefined){localStorage.setItem('ecmNotifGroup',p.notifGroup?'1':'0');_notifGrouping=p.notifGroup;var cb=document.querySelector('.notif-group-toggle');if(cb)cb.checked=p.notifGroup;loadNotifications();}if(p.tileVis&&Object.keys(p.tileVis).length){var existing=loadTilePrefs();var merged={};TILE_IDS.forEach(function(id){merged[id]=p.tileVis[id]!==undefined?p.tileVis[id]:existing[id]!==undefined?existing[id]:true;});saveTilePrefs(merged);}if(p.tileOrder&&p.tileOrder.length){loadTileOrder(p.tileOrder);}}catch(e){}}
